...
export function buildUserPrompt(transcript: string) {
  // Updated to request a French, JSON-structured output: intro, detailed plan, conclusion, and a concise draft.
  return `
...
- Fournis en plus un "brouillon" de texte (bref) qui sera lu par TTS, en français clair, 180–300 mots, en un seul paragraphe continu (sans listes, sans puces, sans Markdown, sans retours à la ligne inutiles).
...
Schéma JSON:
{
  ...
  "draft": "string"                          // Brouillon lisible par TTS, 180–300 mots, français, 1 seul paragraphe, sans listes
}
